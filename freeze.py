...
include_directories = [
    ...
    ('Media', 'Media'),
    ('Models', 'Models'),
    (os.path.join("StyleTTS2", "Utils"), os.path.join("StyleTTS2", "Utils")),
...
for source_dir, target_dir in include_directories:
    for root, dirs, files in os.walk(source_dir):
        for file in files:
            include_files.append((os.path.join(root, file), os.path.join(
                target_dir, os.path.relpath(root, source_dir), file)))
...
executables = [Executable(
    "epubTextToSpeech.py", icon='D:\Python\Epubtextspeech\Imgs\voice_presentation.ico'), "setup.py"]
...
print(
    f"Running cx_freeze with build_options [{build_exe_options}] and executables [{executables}]")
