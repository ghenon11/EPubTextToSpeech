...
class epubTextToSpeech(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkButton(self.controls_frame, text="Synthetize Audio", command=self.convert_to_audio_callthread).grid(row=0, column=1, padx=10, pady=10)
    ...
    def convert_to_audio(self,num,text):
        try:
            logging.info(f"Synthetizing text {text[:50].strip()} {num}")
            if text:
