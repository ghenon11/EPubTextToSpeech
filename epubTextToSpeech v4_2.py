...
class epubTextToSpeech(ctk.CTk):
    def __init__(self):
        ...
        try:
            ...
            self.sync_audio_button = ctk.CTkButton(
                self.scroll_controls, text="Sync Audio to Reading", command=self.sync_audio_to_scroll
            )
    ...
    def read_ebook(self):
        ...
        try:
            ...
            self.book = epub.read_epub(
                config.CURRENT_EBOOK_PATH, {"ignore_ncx": True})
            self.book_title = (
    ...
    def sync_audio_to_scroll(self):
        try:
            ...
            start, _ = self.content_text.yview()
            percent = float(start)
            seek_time = percent * self.audio_duration

            # Pass ratio 0.0 - 1.0
            # start 10s before
            self.seek_audio((seek_time) / self.audio_duration)
            self.pause_audio()
            logging.info(
                f"Audio synced to {int(percent * 100)}% of text: seek_time {seek_time} audio_duration {self.audio_duration}")
        except Exception as e:
    ...
    def pause_audio(self):
        if self.audio_play_obj and self.audio_play_obj.is_playing():
            ...
            self.pause_time = self.audio_progress.get() * self.audio_duration
            self.is_paused = True
            self.stop_flag.set()
    ...
    def convert_to_audio_threaded(self):
        ...
        self.synt_button.configure(fg_color="Green", hover_color="LimeGreen")
        ...
        try:
            ...
            self.synt_button.configure(
                fg_color="#3a7ebf", hover_color="#325882", text="Synthetization completed")
